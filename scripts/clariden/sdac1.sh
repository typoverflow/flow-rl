...
SHARED_ARGS=(
    "algo=sdac" 
    "algo.diffusion.steps=2"
    "log.tag=step_ablation"
    "log.project=flow-rl-ablation"
    "log.entity=gaochenxiao"
)

# Change to repo root (adjust if sbatch is launched from elsewhere)
cd "${SLURM_SUBMIT_DIR:-$(dirname "$0")}"
mkdir -p log

# Use Slurm's GPU visibility: on a single node we get 0,1,2,3
...
run_task() {
    ...
    python3 examples/online/main_mujoco_offpolicy.py task="$task" seed="$seed" algo.diffusion.steps="$step" ${SHARED_ARGS[@]}
}
