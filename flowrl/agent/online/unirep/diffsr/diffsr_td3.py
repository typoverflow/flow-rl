...
def update_critic(
    ...
    t = jnp.ones((batch.obs.shape[0], 1), dtype=jnp.int32) * jnp.int32(1)
    at = jnp.sqrt(diffusion_actor.alpha_hats[t]) * batch.action + jnp.sqrt(1 - diffusion_actor.alpha_hats[t]) * jax.random.normal(eps_rng, batch.action.shape)
    ft = ddpm_target(batch.obs, at, t, method="forward_phi")
    # snr = jnp.sqrt(diffusion_actor.alpha_hats[t] / (1 - diffusion_actor.alpha_hats[t]))
    def value_loss_fn(value_params: Param, dropout_rng: PRNGKey) -> Tuple[jnp.ndarray, Metric]:
        qt_pred = diffusion_value.apply(
            {"params": value_params},
            ft,
        )
    ...
    new_value, value_metrics = diffusion_value.apply_gradient(value_loss_fn)
...
def update_actor(
    ...
    eps_estimation = - sigma * q_grad / 0.1 / (jnp.abs(q_grad).mean() + 1e-6)
...
class DiffSRACAAgent(TD3Agent):
    ...
    def __init__(self, obs_dim: int, act_dim: int, cfg: DiffSRTD3Config, seed: int):
        ...
        #     activation=jax.nn.elu,
        # )
        self.diffusion_value = Model.create(
            value_def,
            diffusion_value_rng,
            inputs=(jnp.ones((1, self.feature_dim)), ),
            optimizer=optax.adam(learning_rate=cfg.diffusion.lr),
